...
class MouseInterpreter(private var parseTree: Node) {
  ...
  // returns the value of a given variable (name)
  def setValueOf(name: String, value: Int): Unit = {
    memory(name) = value
  }
  ...
  def run(stmt: Node): Unit = {
    ...
    // pops the stack twice, let's say b and a (in this order), assigning 'b' to the variable pointed by 'a'
      ...
      val b = stack.pop // this value to be stored
      val address = stack.pop // store the value of b in this address
      setValueOf(getNameOf(address),b)
    }
